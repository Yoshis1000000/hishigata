...
namespace osu.Game.Rulesets.Hishigata.Mods
{
    public class HishigataModApproachDifferent : Mod, IApplicableToDrawableHitObject
    {
        ...
        {
            drawableHitObject.ApplyCustomUpdateState += (DrawableHitObject drawableHitObject, ArmedState _) =>
            {
                if (drawableHitObject is DrawableHishigataNote drawableHishigataNote)
                {
                    ...
                    using (drawableHishigataNote.BeginAbsoluteSequence(hishigataNote.StartTime - hishigataNote.TimePreempt))
                    {
                        if (hishigataNote.IsFeign)
                        {
                            drawableHishigataNote.ClearTransforms(targetMember: nameof(drawableHishigataNote.Rotation));
